...
import { autoRun, suite } from "./helpers.mjs";
...
export async function run() {
  ...
  await s.test("package metadata includes trust and discovery fields", async () => {
    ...
    if (!pkg.files?.includes("docs/AUTHENTICATION.md")) throw new Error("Authentication doc not published");
    if (!pkg.files?.includes("docs/COMPATIBILITY.md")) throw new Error("Compatibility doc not published");
    if (!pkg.files?.includes("docs/COVERAGE_MAP.md")) throw new Error("Coverage map doc not published");
  ...
  await s.test("CHANGELOG documents unreleased and released entries", async () => {
    ...
    for (const section of ["## Unreleased", "## 0.1.0 - 2026-06-17"]) {
